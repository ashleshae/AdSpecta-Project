      ...
      try {
        ...
        if (crowdLevel !== "") {
          filtered = filtered.filter(
            (ad) => ad["Crowd level"]?.toLowerCase() === crowdLevel.toLowerCase()
          );
        }

        if (sortBy === "price-high") {
          filtered.sort((a, b) => parseFloat(a.Starting_Rate || 0) - parseFloat(b.Starting_Rate || 0));
        } else if (sortBy === "price-low") {
          filtered.sort((a, b) => parseFloat(b.Starting_Rate || 0) - parseFloat(a.Starting_Rate || 0));
        }
